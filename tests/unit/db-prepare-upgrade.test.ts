...
test(
  "older SQLite installs upgrade through db:prepare, keep a restore-ready backup, and backfill published content for the current runtime",
  { timeout: 60_000 },
  async () => {
    ...
    try {
      ...
      createLegacyUpgradeFixture(databasePath);

      const env = {
        ...process.env,
        AUTH_SECRET: "minakeep-upgrade-test-secret",
        AUTH_TRUST_HOST: "true",
        DATABASE_URL: databaseUrl,
        MEDIA_ROOT: path.join(tempDirectory, "media"),
        OWNER_USERNAME: "owner",
        OWNER_PASSWORD: "owner-password"
      };
      const prepareResult = await run(process.execPath, ["scripts/db-prepare.mjs"], env);

      assert.match(prepareResult.stdout, /Created pre-upgrade SQLite backup/);
      assert.match(prepareResult.stdout, /Backfilled publishedAt for 1 note\(s\) and 1 link\(s\)/);

      const backupRoots = readdirSync(path.join(tempDirectory, "backups"));
      assert.equal(backupRoots.length, 1);
      const backupDirectory = path.join(tempDirectory, "backups", backupRoots[0]);
      assert.ok(existsSync(path.join(backupDirectory, "minakeep.db")));

      const upgradedDatabase = new Database(databasePath, { readonly: true });
      const backupDatabase = new Database(path.join(backupDirectory, "minakeep.db"), { readonly: true });
      ...
      try {
        ...
        const upgradedNote = upgradedDatabase
          .prepare(`SELECT "title", "slug", "isPublished", "publishedAt" FROM "Note" WHERE "id" = ?`)
          .get("note-1") as { isPublished: number; publishedAt: string | null; slug: string; title: string } | undefined;
        const upgradedLink = upgradedDatabase
          .prepare(`SELECT "title", "isPublished", "publishedAt" FROM "Link" WHERE "id" = ?`)
          .get("link-1") as { isPublished: number; publishedAt: string | null; title: string } | undefined;
        ...
        assert.deepEqual(upgradedNote, {
          isPublished: 1,
          publishedAt: upgradedNote?.publishedAt ?? null,
          slug: "legacy-note",
          title: "Legacy note"
        });
        assert.equal(upgradedNote?.publishedAt === null, false);
        assert.deepEqual(upgradedLink, {
          isPublished: 1,
          publishedAt: upgradedLink?.publishedAt ?? null,
          title: "Legacy link"
        });
        assert.equal(upgradedLink?.publishedAt === null, false);
